...
class MainActivity : AppCompatActivity() {
    ...
    fun scanLeDevice(enable: Boolean, automatic : Boolean = false) {
        ...
        if (enable) {
            ...
            val uuid = UUID.fromString("3c0a1000-281d-4b48-b2a7-f15579a1c38f")
            val scanFilter = ScanFilter.Builder().setServiceUuid(ParcelUuid(uuid)).build()


            bluetoothScanner.startScan(listOf<ScanFilter>(scanFilter), builderScanSettings.build(), leScanCallback)
